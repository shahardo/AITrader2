...
// tokens and routes to onboarding (incomplete setup) or the universe; backend
...
/** Stub fetch: auth endpoints return tokens, onboarding/status returns the given flags. */
function mockApi(onboarding: { complete: boolean }) {
  const mock = vi.fn().mockImplementation((url: string) => {
    if (url.includes('/onboarding/status'))
      return Promise.resolve(new Response(JSON.stringify(onboarding), { status: 200 }))
...
describe('LoginPage', () => {
  it('logs in, stores tokens, and navigates to onboarding when setup is incomplete', async () => {
    mockApi({ complete: false })
    renderLogin()
  ...
  it('navigates straight to the universe when setup is complete', async () => {
    mockApi({ complete: true })
    renderLogin()
    await userEvent.type(screen.getByLabelText(/email/i), 'a@b.com')
    await userEvent.type(screen.getByLabelText(/password/i), 'longenough1')
    await userEvent.click(screen.getByRole('button', { name: /log in/i }))
  ...
  it('skips the wizard for accounts that dismissed onboarding', async () => {
    dismissOnboarding('a@b.com')
    mockApi({ complete: false })
    renderLogin()
    await userEvent.type(screen.getByLabelText(/email/i), 'a@b.com')
    await userEvent.type(screen.getByLabelText(/password/i), 'longenough1')
